...
namespace IBSC.UI.Web.Page
{
    public partial class InsuranceCheck : System.Web.UI.Page
    {
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                DataTable dt = insureCarDal.GetAllCondition(ddlCarYear.Text, ddlCarName.Text, ddlCarModel.Text, ddlCarEngine.Text);
        ...
        protected void ddlCarYear_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                ddlCarEngine.Items.Clear();
                ddlCarEngine.Items.Add(new ListItem("กรุณาเลือก", "กรุณาเลือก"));
                if (dt != null)
                {
                    foreach (DataRow row in dt.Rows)
                    {
        ...
        protected void ddlCarModel_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                DataTable dt = insureCarDal.GetComboBoxCarEngine(ddlCarYear.Text, ddlCarName.Text, ddlCarModel.Text);
                ...
                if (dt != null)
                {
                    foreach (DataRow row in dt.Rows)
                    {
                        ddlCarEngine.Items.Add(new ListItem(row[0].ToString(), row[0].ToString()));
                    }
                    ddlCarEngine.DataBind();
                }
            }
            catch (Exception ex)
            {
